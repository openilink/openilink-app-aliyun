 ...
 *
 * 流程：
 * 1. GET  /oauth/setup → 显示配置表单 HTML（填写 AccessKey ID / Secret / 区域）
...
<body>
  <div class="card">
    <h1>阿里云</h1>
    <p class="desc">请填写您的阿里云 AccessKey，用于连接阿里云 API</p>
    <form method="POST" action="/oauth/setup?hub=${encodeURIComponent(hub)}&app_id=${encodeURIComponent(appId)}&bot_id=${encodeURIComponent(botId)}&state=${encodeURIComponent(state)}&return_url=${encodeURIComponent(returnUrl)}">
      <label class="required">AccessKey ID</label>
      <input name="aliyun_access_key_id" placeholder="LTAI5txxxxxxxxxxxxxxxxxx" required />
      <p class="hint">在 <a href="https://ram.console.aliyun.com/manage/ak" target="_blank">阿里云 AccessKey 管理</a> 创建</p>

      <label class="required">AccessKey Secret</label>
      <input name="aliyun_access_key_secret" type="password" placeholder="AccessKey 密钥" required />

      <label>区域（可选）</label>
      <input name="aliyun_region" placeholder="cn-hangzhou" value="cn-hangzhou" />
      <p class="hint">默认 cn-hangzhou，可填写 cn-beijing、cn-shanghai 等</p>
