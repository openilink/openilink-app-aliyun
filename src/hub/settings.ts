/**
 * /settings 页面 — 允许用户通过 app_token 验证身份后查看和修改阿里云凭证配置
 */
...
export async function handleSettingsSave(
  ...
  const akId = formData.get("aliyun_access_key_id") || "";
  const akSecret = formData.get("aliyun_access_key_secret") || "";
  const region = formData.get("aliyun_region") || "";
  ...
  if (akId) newConfig.aliyun_access_key_id = akId;
  if (akSecret) newConfig.aliyun_access_key_secret = akSecret;
  // 区域允许清空（使用默认值），所以特殊处理
  newConfig.aliyun_region = region;
...
function renderConfigForm(
  ...
  const maskedAkId = maskValue(cfg.aliyun_access_key_id || "");
  const maskedAkSecret = maskValue(cfg.aliyun_access_key_secret || "");
  const currentRegion = cfg.aliyun_region || "cn-hangzhou";
...
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <title>阿里云 — 修改配置</title>
  <style>${COMMON_STYLE}</style>
</head>
<body>
  <div class="card">
    <h1>阿里云设置</h1>
    <p class="desc">修改您的阿里云 AccessKey 配置</p>
    ${successMsg ? `<div class="success">${successMsg}</div>` : ""}

    <div class="current-config">
      <dt>当前 AccessKey ID</dt>
      <dd>${maskedAkId || "（未配置）"}</dd>
      <dt>当前 AccessKey Secret</dt>
      <dd>${maskedAkSecret || "（未配置）"}</dd>
      <dt>当前区域</dt>
    ...
    <form method="POST" action="/settings/save">
      ...
      <input name="aliyun_access_key_id" placeholder="留空则不修改" />
      <p class="hint">在 <a href="https://ram.console.aliyun.com/manage/ak" target="_blank">阿里云 AccessKey 管理</a> 查看</p>

      <label>AccessKey Secret</label>
      <input name="aliyun_access_key_secret" type="password" placeholder="留空则不修改" />

      <label>区域</label>
      <input name="aliyun_region" placeholder="留空则使用默认值 cn-hangzhou" value="${escapeHtml(cfg.aliyun_region || "")}" />
      <p class="hint">默认 cn-hangzhou</p>
      ...
      <div class="security-notice">
        ...
        <ul>
          ...
          <li>如需更高安全性，建议<a href="https://github.com/openilink/openilink-app-aliyun">自行部署</a>本 App</li>
        </ul>
      </div>
