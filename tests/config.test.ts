...
describe("loadConfig", () => {
  ...
  it("缺少所有必填项时应抛出错误，并列出全部缺失项", () => {
    expect(() => loadConfig()).toThrowError(
      "缺少必填环境变量: HUB_URL, BASE_URL, ALIYUN_ACCESS_KEY_ID, ALIYUN_ACCESS_KEY_SECRET",
    );
  });

  it("缺少 HUB_URL 时应抛出错误", () => {
    process.env.BASE_URL = "http://app.test";
    process.env.ALIYUN_ACCESS_KEY_ID = "test-id";
    process.env.ALIYUN_ACCESS_KEY_SECRET = "test-secret";
    expect(() => loadConfig()).toThrowError("HUB_URL");
  });

  it("缺少 ALIYUN_ACCESS_KEY_ID 时应抛出错误", () => {
    process.env.HUB_URL = "http://hub.test";
    process.env.BASE_URL = "http://app.test";
    process.env.ALIYUN_ACCESS_KEY_SECRET = "test-secret";
    expect(() => loadConfig()).toThrowError("ALIYUN_ACCESS_KEY_ID");
  });

  it("缺少 ALIYUN_ACCESS_KEY_SECRET 时应抛出错误", () => {
    process.env.HUB_URL = "http://hub.test";
    process.env.BASE_URL = "http://app.test";
    process.env.ALIYUN_ACCESS_KEY_ID = "test-id";
    expect(() => loadConfig()).toThrowError("ALIYUN_ACCESS_KEY_SECRET");
  });
  ...
  it("提供所有必填项后应正确加载默认值", () => {
    ...
    expect(cfg.dbPath).toBe("data/aliyun.db");
    expect(cfg.aliyunRegion).toBe("cn-hangzhou");
  ...
  it("所有环境变量应正确映射到配置对象", () => {
    ...
    process.env.ALIYUN_ACCESS_KEY_ID = "LTAI5tTestKeyId";
    process.env.ALIYUN_ACCESS_KEY_SECRET = "TestSecretValue123";
    ...
    expect(cfg.aliyunAccessKeyId).toBe("LTAI5tTestKeyId");
    expect(cfg.aliyunAccessKeySecret).toBe("TestSecretValue123");
    expect(cfg.aliyunRegion).toBe("cn-beijing");
  });
});
